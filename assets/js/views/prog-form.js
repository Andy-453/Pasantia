...
+'<div class="grid2"><div class="field" id="llbox'+l.id+'"'+(l.t===TIPO_SIN_LINEA?' style="display:none"':'')+'><label>Línea</label><input id="ll'+l.id+'"'+(l.t===TIPO_SIN_LINEA?' disabled':'')+' value="'+esc(l.l||'')+'" placeholder="Nombre de la línea"></div>'
+'<div class="field" id="lmbox'+l.id+'"'+(l.t===TIPO_SIN_LINEA?'':' style="display:none"')+'><label>Motivo / modalidad</label><input id="lm'+l.id+'"'+(l.t===TIPO_SIN_LINEA?'':' disabled')+' value="'+esc(l.motivo||'')+'" placeholder="Ej: Semestre avanzado"></div>'
+'<div class="field"><label>Tipo</label><select id="lt'+l.id+'" data-action="prog-linea-tipo"><option'+(l.t==='Profundización 1'?' selected':'')+'>Profundización 1</option><option'+(l.t==='Profundización 2'?' selected':'')+'>Profundización 2</option><option'+(l.t==='Profundización 3'?' selected':'')+'>Profundización 3</option><option'+(l.t===TIPO_SIN_LINEA?' selected':'')+'>'+TIPO_SIN_LINEA+'</option></select></div></div>'
      +'<div class="grid2"><div class="field"><label>Especialización</label><input id="le'+l.id+'" value="'+esc(l.esp||'')+'" placeholder="Nombre"></div>'
      ...
      +'<div class="field"><label>👤 Responsable</label><input id="lresp'+l.id+'" value="'+esc(l.resp||'')+'" placeholder="Docente o equipo"></div></div>'
      +'<div class="grid3"><div class="field"><label>📅 Mes</label><select id="lmes'+l.id+'">'+mo(l.mes)+'</select></div>'
+'<div class="field"><label>📅 Año</label><select id="lano'+l.id+'">'+ao(l.ano)+'</select></div><div class="field"><label>🔗 Enlace</label><input type="url" id="lenlace'+l.id+'" value="'+esc(l.enlaceObtencion||'')+'" placeholder="URL programa"></div></div>'
...
+'</div>';
  ...
  var mH=tmpMaes.map(function(m){
    return '<div class="linea-card" id="mc'+m.id+'">'
      ...
      +'<div class="grid2"><div class="field"><label>Maestría</label><input id="mn'+m.id+'" value="'+esc(m.n||'')+'" placeholder="Nombre"></div>'
      ...
      +'<div class="field"><label>👤 Responsable</label><input id="mresp'+m.id+'" value="'+esc(m.resp||'')+'" placeholder="Docente o equipo"></div></div>'
      +'<div class="grid3"><div class="field"><label>📅 Mes</label><select id="mmes'+m.id+'">'+mo(m.mes)+'</select></div>'
+'<div class="field"><label>📅 Año</label><select id="mano'+m.id+'">'+ao(m.ano)+'</select></div><div class="field"><label>🔗 Enlace</label><input type="url" id="menlace'+m.id+'" value="'+esc(m.enlaceObtencion||'')+'" placeholder="URL programa"></div></div>'
